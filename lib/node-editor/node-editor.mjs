...
export class NodeEditor {
    ...
    startInputConnection(sourceField) {
    ...
    finishInputConnection(fromId, toId) {
        const fields = this.getAllFields();
    ...
    static fromJSON(parse) {
        const types = parse.nodeTypes.map(type => new NodeType(type.name, type.fields.map(field => {
            ...
            return new InputField(field.name, fieldType, field.default, field.required, field.shown, field.connections, field.id);
        })));
        ...
        const globals = parse.globals.map(global => {
            return new GlobalSection(global.name, global.values, global.fields.map(field => {
                const fieldType = Object.values(ValueTypes).find(type => type.name === field.type.name);
                return new InputField(field.name, fieldType, field.default, field.required, field.shown, field.connections, field.id);
            }));
        });
        return new NodeEditor(types, nodes, globals);
    }
}
